...
havoc_stage:
  ...
  // u32 *mutation_array;
  u32 stack_max;
  // u32 rand_max;  // stack_max_pow = afl->havoc_stack_pow2;
  ...
  /*
    switch (afl->input_mode) {
      ...
      case 2: {  // BINARY
        ...
        if (likely(afl->fuzz_mode == 0)) {  // is exploration?
          mutation_array = (unsigned int
    *)&mutation_strategy_exploration_binary; rand_max = MUT_STRATEGY_ARRAY_SIZE;

        } else {  // exploitation mode

          mutation_array = (unsigned int
    *)&mutation_strategy_exploitation_binary; rand_max =
    MUT_STRATEGY_ARRAY_SIZE;
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    item); strcat(afl->mutation, afl->m_tmp); #endif out_buf[rand_below(afl,
    temp_len)] = interesting_8[item]; break;
    ...
    item); strcat(afl->mutation, afl->m_tmp); #endif
    ...
    #ifdef INTROSPECTION
              snprintf(afl->m_tmp, sizeof(afl->m_tmp), " ARITH16BE+__%u",
    num); strcat(afl->mutation, afl->m_tmp); #endif
              *(u16 *)(out_buf + pos) =
                  SWAP16(SWAP16(*(u16 *)(out_buf + pos)) + num);
            ...
            case MUT_CLONE_COPY: {
              ...
              if (likely(temp_len + HAVOC_BLK_XL < MAX_FILE)) {
                ...
                strcat(afl->mutation, afl->m_tmp);
    #endif
    if (AFL_BUF_PARAM(out_scratch) == AFL_BUF_PARAM(out)) fprintf(stderr,
    "SAME!\n"); fprintf(stderr, "%p %p\n", AFL_BUF_PARAM(out_scratch),
    AFL_BUF_PARAM(out)); u8 *new_buf = afl_realloc(AFL_BUF_PARAM(out_scratch),
    temp_len + clone_len); if (unlikely(!new_buf)) { PFATAL("alloc"); }
                ...
                // Tail
    fprintf(stderr, "dst=%p + %x + %x  src=%p + %x  len=%x\n", new_buf,
    clone_to, clone_len, out_buf, clone_to, temp_len - clone_to);
    ...
    OVERWRITE-COPY_%u_%u_%u", copy_from, copy_to, copy_len);
              strcat(afl->mutation, afl->m_tmp);
    #endif
    ...
    "SAME!\n"); fprintf(stderr, "%p %p\n", AFL_BUF_PARAM(out_scratch),
    AFL_BUF_PARAM(out)); u8 *new_buf = afl_realloc(AFL_BUF_PARAM(out_scratch),
    switch_len); if (unlikely(!new_buf)) { PFATAL("alloc"); }
    ...
    #ifdef INTROSPECTION
              snprintf(afl->m_tmp, sizeof(afl->m_tmp), " DELONE_%u",
    del_from); strcat(afl->mutation, afl->m_tmp); #endif memmove(out_buf +
    del_from, out_buf + del_from + del_len, temp_len - del_from - del_len);
    ...
    strat, clone_to); strcat(afl->mutation, afl->m_tmp); #endif u8 *new_buf =
                  afl_realloc(AFL_BUF_PARAM(out_scratch), temp_len +
    clone_len); if (unlikely(!new_buf)) { PFATAL("alloc"); }
    ...
    if (AFL_BUF_PARAM(out_scratch) == AFL_BUF_PARAM(out)) fprintf(stderr,
    "SAME!\n"); u8 *new_buf = afl_realloc(AFL_BUF_PARAM(out_scratch), temp_len
    + new_len - old_len); if (unlikely(!new_buf)) { PFATAL("alloc"); }
                ...
                // Tail
                memcpy(new_buf + off + new_len, out_buf + off2, temp_len -
    off2);
    ...
    #ifdef INTROSPECTION
              snprintf(afl->m_tmp, sizeof(afl->m_tmp), "
    EXTRA-OVERWRITE_%u_%u", insert_at, extra_len); strcat(afl->mutation,
    afl->m_tmp); #endif memcpy(out_buf + insert_at,
    afl->extras[use_extra].data, extra_len);
    ...
    #ifdef INTROSPECTION
              snprintf(afl->m_tmp, sizeof(afl->m_tmp), "
    AUTO-EXTRA-INSERT_%u_%u", insert_at, extra_len); strcat(afl->mutation,
    afl->m_tmp); #endif
    ...
    temp_len + clone_len + 1); if (unlikely(!temp_buf)) { PFATAL("alloc"); }
    fprintf(stderr, "%p %p\n", AFL_BUF_PARAM(out_scratch),
    AFL_BUF_PARAM(out));

    #ifdef INTROSPECTION
              snprintf(afl->m_tmp, sizeof(afl->m_tmp), "
    SPLICE-INSERT_%u_%u_%u_%s", clone_from, clone_to, clone_len,
    target->fname); strcat(afl->mutation, afl->m_tmp); #endif
              // Head
    ...
    fprintf(stderr, "dst=%p + %x  src=%p + %x  len=%x\n", temp_buf, clone_to,
    new_buf, clone_from, clone_len); memcpy(temp_buf + clone_to, new_buf +
...
skip_interest:
  ...
  for (i = 0; i < (u32)len; ++i) {
    ...
    for (j = 0; j < afl->extras_cnt; ++j) {
      ...
      /* Skip extras probabilistically if afl->extras_cnt >
         AFL_MAX_DET_EXTRAS. Also skip them if there's no room to insert the
         payload, if the token is redundant, or if its entire span has no
         bytes set in the effector map. */
  ...
  // for (; afl->swarm_now < swarm_num; ++afl->swarm_now)
  {
          ...
          switch (r = (select_algorithm(afl, r_max))) {
            ...
            case 14:
              ...
              if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
                ...
                strcat(afl->mutation, afl->m_tmp);
#endif
                if (AFL_BUF_PARAM(out_scratch) == AFL_BUF_PARAM(out))
                  fprintf(stderr, "SAME!\n");
    ...
    retry_splicing_puppet:
      ...
      if (afl->use_splicing &&
        ...
        /* Pick a random queue entry and seek to it. Don't splice with
         * yourself.
         */
        ...
        /* Find a suitable splicin g location, somewhere between the first and
           the last differing byte. Bail out if the difference is just a
           single byte or so. */
    ...
    abandon_entry_puppet:
      ...
      /*
        ...
        if (!afl->stop_soon && !afl->queue_cur->cal_failed &&
            (afl->queue_cur->was_fuzzed == 0 || afl->queue_cur->fuzz_level ==
        0)
